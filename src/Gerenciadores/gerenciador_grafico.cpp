...
Gerenciador_Grafico::Gerenciador_Grafico()
    : width(1280),
      height(920)  // Inicialização das variáveis membro
{
  inicializador();
}

// Destrutor
Gerenciador_Grafico::~Gerenciador_Grafico() { shutdown(); }
...
// Fechar a janela
void Gerenciador_Grafico::shutdown() { window.close(); }
...
void Gerenciador_Grafico::desenharEnte(Ente* pE) {
  ...
  if (!texture) {
    ...
    if (!texturaPadrao.loadFromFile("textura_padrao.png")) {
      std::cerr << "Erro: Não foi possível carregar a textura padrão!"
                << std::endl;
      return;
    }
    ...
    std::cerr << "A textura padrão foi aplicada ao sprite do Ente."
              << std::endl;
  }
