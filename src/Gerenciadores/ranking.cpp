...
Ranking::Ranking() : dados() { dados.setLocal("saves/rank.json"); }
...
Entidades::Personagens::Tripulante* Ranking ::getJogador() { return pJog1; }
...
void Ranking::carregarJson(Dados& dados) {
  ...
  if (arquivo.is_open()) {
    ...
    cout << "Dados carregados: " << dados.getNome()
         << ", Pontos: " << dados.getPontos() << ", Posição: ("
         << dados.getPos().first << ", " << dados.getPos().second << ")"
         << endl;
