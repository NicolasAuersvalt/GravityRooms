...
void Save::salvar( Gerenciador_Colisoes& GC,  Lista_Entidades& listaPersonagem,  Lista_Entidades& listaObstaculo,  Fase& fase) {
    nlohmann::ordered_json saveData;

    // Save level state
    saveData["level"] = dynamic_cast<const Laboratorio*>(&fase) ? "laboratorio" : "nave";
...
bool Save::carregar( Gerenciador_Colisoes& GC,  Lista_Entidades& listaPersonagem,  Lista_Entidades& listaObstaculo,  Fase*& fase) {
  try {
        ...
        IDs::IDs faseID = (level == "laboratorio") ? IDs::IDs::fase_laboratorio : IDs::IDs::fase_nave;
        fase = (faseID == IDs::IDs::fase_laboratorio) ? static_cast<Fase*>(new Laboratorio(faseID)) : static_cast<Fase*>(new Nave(faseID));
