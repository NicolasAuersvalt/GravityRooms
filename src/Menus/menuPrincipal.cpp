...
MenuPrincipal::MenuPrincipal(IDs::IDs id)
    : Menu(id, Vector2f(TAMANHO_BOTAO_X, TAMANHO_BOTAO_Y), "MenuPrin",
           180) {
  titulo.setPos(
...
void MenuPrincipal::criarBotoes() {
  ...
  addBotao("LABORATORIO", Vector2f(centerX, startY),
           IDs::IDs::botao_novoJogo, Color(0, 255, 0));
...
void MenuPrincipal::criarFundo() {
  if (!texturaBackground.loadFromFile("assets/logo.png")) {
    cerr << "Erro ao carregar textura do background do menu principal!"
              << endl;
    exit(1);
  }
